...
# Expanding fugure 2 to incude all the treatments
# Dario Trujano-Ochoa
...
# All treatments ----
treatment <- c("Information",
               "RCL",
               'No-Information',
               "Feedback",
               "Description")

# Analysis by round within the proirs ----
for (t in 1:length(treatment)) {
  # data for BSR (elicitation method) and RLC treatment
  bsr <-
    data.bsr.qsr %>% 
    filter(scoringrule == 1, 
           treatment == t)
  # there are 59 participant in BSR-rlc condition
  bsr %>% distinct(subjectid)
  ...
  bsr %>% 
    ...
    theme_minimal_hgrid() +
    labs(
      title = paste(treatment[t]),
      x = "Round", 
      y = "Fraction of false reports", family = "bell") +
    scale_color_discrete(name = "Prior") 
  
  ggsave(paste("results/rounds_all_priors_", treatment[t],".pdf",sep = ""),
         width=6, height=5, pointsize = 5)
  ...
  ggsave(paste("results/rounds_stated_prior_", treatment[t],".pdf",sep = ""),
         width=6, height=5, pointsize = 5)
  ...
  #join both proportion in a single table
  ...
  ggsave(paste("results/fig2A_round_one_", treatment[t],".pdf",sep = ""),
  ...
  ggsave(paste("results/fig2B_round_one_", treatment[t],".pdf",sep = ""),
         width=s*g, height=s, pointsize = s)
