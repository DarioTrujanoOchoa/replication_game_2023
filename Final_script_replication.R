...
########################################
##########Priors by Treatments##########
########################################

table(data$pur,data$t_cats)
...
# Select prior reports from BSR Information treatment
data_panel_a_20 <- data[Main_data.df$scoringrule == 1 & data$treatment == 1 & data$pur==20, ] ### 

data_panel_a_20 <- data_panel_a_20[, c("period", "false_bool")]

# Get period averages and confidence intervals
data_panel_a_summary_20.df <- aggregate(false_bool ~ period, data = data_panel_a_20, FUN = function(x) {
  c(mean = mean(x), sd = sd(x), count = length(x))
})

write.csv(data_panel_a_summary_20.df, "Panel1_sum_20.csv") ### The format was acting up

data_panel_a_summary1__20 <- read.csv("Panel1_sum_20.csv", header = TRUE)
...
data_panel_a_summary1__20$low <- data_panel_a_summary1__20$false_bool.mean - qt(0.025, data_panel_a_summary1__20$false_bool.count - 1) * (data_panel_a_summary1__20$false_bool.sd / sqrt(data_panel_a_summary1__20$false_bool.count))

# Create the figure
library(ggplot2)
figure2_A_20 <- ggplot(data_panel_a_summary1__20, aes(x = period, y = false_bool.mean, ymin = low, ymax = up)) +
  geom_point(shape = 3, size = 2) +
  geom_line()+
  geom_errorbar(width = 0.1) +
  scale_x_continuous(breaks=seq(0,10,by=1))+
  ...
  labs(
    x = "Period",
    y = "Fraction of False Reports for 0.20",
    title = "Figure 2 Panel A [P=0.20]"
  )
...
# Select prior reports from BSR Information treatment
data_panel_a_30 <- data[Main_data.df$scoringrule == 1 & data$treatment == 1 & data$pur==30, ] ### 

data_panel_a_30 <- data_panel_a_30[, c("period", "false_bool")]

# Get period averages and confidence intervals
data_panel_a_summary_30.df <- aggregate(false_bool ~ period, data = data_panel_a_30, FUN = function(x) {
  c(mean = mean(x), sd = sd(x), count = length(x))
})

write.csv(data_panel_a_summary_30.df, "Panel1_sum_30.csv") ### The format was acting up

data_panel_a_summary1__30 <- read.csv("Panel1_sum_30.csv", header = TRUE)
...
data_panel_a_summary1__30$low <- data_panel_a_summary1__30$false_bool.mean - qt(0.025, data_panel_a_summary1__30$false_bool.count - 1) * (data_panel_a_summary1__30$false_bool.sd / sqrt(data_panel_a_summary1__30$false_bool.count))

# Create the figure
library(ggplot2)
figure2_A_30 <- ggplot(data_panel_a_summary1__30, aes(x = period, y = false_bool.mean, ymin = low, ymax = up)) +
  geom_point(shape = 3, size = 2) +
  geom_line()+
  geom_errorbar(width = 0.1) +
  scale_x_continuous(breaks=seq(0,10,by=1))+
  ...
  labs(
    x = "Period",
    y = "Fraction of False Reports for 0.30",
    title = "Figure 2 Panel A [P=0.30]"
  )
